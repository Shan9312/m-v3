<template>
  <div>
    ...
    <div class="list">
      <ul>
        <li
          v-for="(item, index) in cardBuyList"
          :key="index"
          class="clearfix"
          @click="
            index_click(item.name, index + 1);
            link(item.countdownTime, item.id);
          "
        >
          <div class="fl view">
            <img
          ...
          <div class="fl describe">
            ...
            <div
              ...
              v-if="
                item.countdownTime &&
                  time_post(item.countdownTime).countDownState
              "
            >
              ...
              {{
                "倒计时：" +
                  time_post(item.countdownTime).htmlHour +
                  ":" +
                  time_post(item.countdownTime).minute +
                  ":" +
                  time_post(item.countdownTime).seconds
              }}
              <!--{{ item.countdownTime}}-->
            </div>
            <div v-else class="countDown fff"></div>
            <br />
            <div class="price">
          ...
          >
            {{
              item.countdownTime && time_post(item.countdownTime).countDownState
                ? "即将开始"
                : "立即订购"
            }}
          </div>
