<template>
  <div class="view">
    ...
    <div class="charge tab_list">
      <div class="banner" v-show="click != -1" :class="click == 1 ? 'banner7' : 'banner30'"></div>
      <div class="input input_view">
        <input
          data-role="none"
          type="tel"
          placeholder="请输入摩拜绑定的手机号"
          v-model="mobile"
          maxlength="13"
        />
      ...
      <ul>
        <li
          v-for="(list, index) in list.mobike_sku_list"
          :key="index"
          class="check"
          :class="{ click: click == index }"
          @click="checklist(index)"
          v-show="list.specification != '1次骑行券'"
        >
          <span>{{ parseInt(list.price) }}元</span>
          <br />
          <span>{{ list.specification }}</span>
        </li>
      </ul>
      <div class="hint" v-show="click == -1">
        <div class="title">充值使用说明</div>1、请填写摩拜APP绑定的手机号。
        <br />2、直接充值到手机号绑定的摩拜账号内，每次骑行抵扣，如果券提前消费完毕，则需要再次充值。
        <br />3、如遇到充值失败，我们将自动进行退款处理。
        <br />4、充值的骑行抵扣券只能在摩拜APP中使用。
      </div>
      <div class="hint" v-show="click != -1">
        <div class="title">充值使用说明</div>
        <span v-show="click == 0">
          1、1张1元骑行券充入账户，骑行券从充入之时算起需要在30天内使用完。
          <br />
        </span>
        <span v-show="click == 1">
          1、7张1元骑行券充入账户，骑行券从充入之时算起需要在7天内使用完。
          <br />
        </span>
        <span v-show="click == 2">
          1、30张1元骑行券充入账户，骑行券从充入之时算起需要在30天内使用完。
          <br />
        </span>
        2、骑行券使用完或过期，均视为失效。
        <br />3、每次行程只能用一张骑行券，免费时长参照摩拜单车当地的计费规则，超出部分按照正常计费规则计费。同一用户每天使用骑行券的次数不设定上限。
        <br />4、仅限中国境内使用。
        <br />5、购买成功后，相应费用不予退还。
        <br />6、如果客户目前有正在激活期的月卡、季卡、半年卡、年卡等产品，本产品充入后和已有产品的有效期不延续计算。
        <br />7、部分地区车辆仅限使用美团APP扫码。
      </div>
    ...
    <footer>
      <div class="integral">
        可用积分：
        <span>{{ list.integral.toFixed(2) }}</span>
      </div>
      <div
        class="button"
        :class="{ check: newMobile.length == 11 && click != -1 }"
        @click="recharge_refer()"
      >立即充值</div>
    </footer>
  </div>
</template>
...
<style scoped>
.banner7 {
  background: url(../../../assets/images/recharge/mobikeCard7.png) 0 0.2rem
    no-repeat;
}
.banner30 {
  background: url(../../../assets/images/recharge/mobikeCard30.png) 0 0.2rem
    no-repeat;
}
.banner {
  width: 6.7rem;
