<template>
  <div>
    <div data-role="page" style="padding-bottom:1rem;text-align: center;font-size: 0;">
        ...
        <div class="formParams">
          ...
          <div class="clearfix">
            ...
            <div class="fr" @click="appeal()">
              ...
              <img
                v-if="arrowImg"
                src="../../assets/images/personalCenter/customerService/down.png"
              />
          ...
          <div class="list_view">
            ...
            <div v-if="dataType==1">
              <div
                v-for="(item,index) in orderAppeal"
                :key="index"
                v-bind:class="{check:item.ifSelect==selected}"
                @click="appealList(item.ifSelect,item.value)"
                class="list_li"
              >
                <span></span>
                <b>{{item.value}}</b>
            ...
            <div v-else-if="dataType==2">
              <div
                v-for="(item,index) in pointAppeal"
                :key="index"
                v-bind:class="{check:item.ifSelect==selected}"
                @click="appealList(item.ifSelect,item.value)"
                class="list_li"
            ...
            <div v-else-if="dataType==6">
              <div
                v-for="(item,index) in serviceAppeal"
                :key="index"
                v-bind:class="{check:item.ifSelect==selected}"
                @click="appealList(item.ifSelect,item.value)"
                class="list_li"
            ...
            <ul>
              <li v-show="newAddImg" v-for="(item,index) in imgList" :key="index">
                <img
                  v-if="item.deleteImg"
                  class="close"
                  src="../../assets/images/personalCenter/customerService/close_complaint.png"
                  @click="delImg(index)"
                />
              ...
              <li @click="chooseImgAPP">
                ...
                <div v-if="defaultImg">
                  <img
                    class="append"
                    src="../../assets/images/personalCenter/customerService/append.jpg"
                  />
                </div>
              </li>
            </ul>
      ...
      <p class="frm_view">
        <span>客服热线</span>
        <a @click="dial_phone()" href="#">400-158-2212</a>
        <br />
        {{serviceTime}}
      </p>
    </div>
  ...
  methods: {
    ...
    chooseImg(e) {
      ...
      if (this.selectImgNum + this.imgNum > this.maxImgNum) {
        ...
        this.selectImgNum + this.imgNum <= this.maxImgNum &&
        this.selectImgNum + this.imgNum > 0
      ) {
        for(let i =0;i<this.selectImgNum;i++){
           this.getOssMsg(e.target.files[i]);
        }
        if (this.selectImgNum + this.imgNum == this.maxImgNum) {
    ...
    ref() {
      ...
      this.imgList.forEach(item =>{
        imagePaths=`${item.imgUrl};${imagePaths}`
      })
      let formData = new FormData();
...
form .list_view .check span {
  background: url(../../assets/images/personalCenter/customerService/check.png)
    no-repeat 0 0;
  background-size: 100%;
  width: 0.3rem;
  height: 0.3rem;
