  ...
  <div class="container">
        ...
        >
          <ul class="tabs1_usableUl">
            <li class="clearfix div_con" v-for="(item,index) in listData" :key="index">
              ...
              <div class="fr con_money">+{{item.cardMoney.toFixed(2)}}</div>
            </li>
    ...
    <div class="isSuccess_bg approve" v-show="nonentityVisible2">
      <div class="isSuccess">
        ...
        <p class="min">
          请联系
          <span class="span_red">400-158-2212</span>
        </p>
        <div class="input_view">
    ...
    <div class="isSuccess_bg approve" v-if="popupVisible3">
      <div class="isSuccess">
        ...
        <p class="min">
          请联系
          <span class="span_red" @click="mobileCall()">400-158-2212</span>
        </p>
        <div class="input_view">
...
export default {
  ...
  methods: {
    jumpCard() {
      dooolyAPP.gotoJumpVue(this.$router, "/card_question");
    },
    inputKeyDown(event) {
      if (event.keyCode == 8) {
...
.div_header {
  ...
  width: 6.9rem;
  margin: auto;
  background: url(../../assets/images/point_recharge/point_recharge_bg.png)
    no-repeat;
  background-size: 100% 100%;
}
.div_header .left {
  margin: 0.54rem 0 0.36rem 0.36rem;
  width: 60%;
  overflow: hidden;
