      ...
      methods:{
        ...
        Wechatshare(prefix,params){
          ...
          if (ua.match(/MicroMessenger/i) == 'micromessenger') {
            client = 'wechat';//是微信
            this.$jsonp(api.commonShareJSONPConfig+"?client="+client+"&prefix="+prefix+"&params="+encodeURI(encodeURI(params.title))+"&params="+encodeURI(encodeURI(params.digest))+"&params="+encodeURIComponent(window.location.href)+"&params="+encodeURI(encodeURI(params.imgUrl))+"&url="+encodeURIComponent(window.location.href),
              {//请求参数
            ...
            client = 'doooly';//不是微信
            this.$jsonp(api.commonShareJSONPConfig+"?client="+client+"&prefix="+prefix+"&params="+encodeURI(encodeURI(localStorage.articletitle))+"&params="+encodeURI(encodeURI(localStorage.articledigest))+"&params="+encodeURIComponent(window.location.href)+"&params="+encodeURI(encodeURI(localStorage.articleimgUrl))+"&url="+encodeURIComponent(window.location.href),
              {//请求参数
                callbackName: 'jsonpCallback',
...
.title_div{color: #000;padding-left: 0.2rem;font-size: 0.36rem;font-weight: 700;}
</style>
