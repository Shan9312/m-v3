  ...
  <div class="view">
    ...
    <header v-show="!first_card_show">
      <ul>
        <li
          ...
          :key="index"
          @click="tab_change(index)"
          :class="{check: index == tab_number}"
        >{{data.title}}</li>
      </ul>
    </header>
    ...
    <div class="charge tab_list" v-show="tab_number == '0'">
      <div class="input input_view">
        ...
        <div @click="mobile = ''" class="input_delete" v-show="mobile">
          <img src="../../../assets/images/recharge/input_delete.png" />
        </div>
      </div>
      <ul>
        <li
          v-for="(list,index) in charge_list"
          class="check"
          @click="pirce_check(list.id,list.price,1)"
          v-show="operator !=''"
        >
      ...
      <div class="coupon_div_bg" v-show="isCoupon">
        <div class="coupon_div">
          ...
          <div class="coupon_fr" v-show="isCoupon_yes" @click="jumpMyCoupon()">
            <span>{{couponNum}}</span>张可用
            <div class="img_div">
              <img src="../../../../src/assets/images/personalCenter/rightArrow.png" />
            </div>
          </div>
          <div class="coupon_fr isCoupon_no_color" v-show="isCoupon_no">
            暂无可用
      ...
      <!-- 显示抵扣券张数 end -->
      <div class="hint">
        <div class="title">充值使用说明</div>1、话费充值到账时间为5-20分钟
        <br />2、月初月末为充值高峰，话费到账可能会延迟或者失败，建议错时充值
        <br />
        3、{{charge_text}}
      </div>
    </div>
    ...
    <div class="rateOfFlow tab_list" v-show="tab_number == '1'">
      ...
      <ul>
        <li
          v-for="(list,index) in rateOfFlow_list"
          class="check"
          @click="pirce_check(list.id,list.price,2)"
          v-show="operator !=''"
        >
          <span>{{list.specification}}</span>
          <br />
      ...
      <div class="hint">
        <div class="title">充值使用说明</div>1、根据相关规定，对于部分如非3G号码/欠费/非实名制/运营商黑名单用户，暂时不能使用流量充值服务。对于实名制等业务的办理方法，建议联系归属地运营商。
        <br />2、充值流量当月有效，月底失效，到账后立即生效。请以短信通知及在运营商处查询的信息为准。
        <br />3、由于联通存在每个面额每个月充值次的限制，充值超过5次，将会出现失败退款。建议您充值其他面额的流量。另外，号码欠费、套餐互斥、非实名认证、运营商黑名单等原因也会导致充值失败退款。
        <br />4、充值失败时我们会立即为您办理退款，如果您使用微信支付，退款会退回微信钱包。信用卡退款时间可能会较长，您可以直接致电银行或登录网银查看退款情况。
        <!-- <br> -->
        <!-- 5、{{rateOfFlow_text}} -->
      </div>
    </div>
    ...
    <div class="card tab_list" v-show="tab_number == '2' && recharge == 0 && !first_card_show">
      <div class="input input_view">
        ...
        <div class="flow_icon" @click="flow_bg_show = !flow_bg_show" v-show="!cardno">
          <img src="../../../assets/images/recharge/flow.png" />
        </div>
        <div @click="cardno = ''" class="input_delete" v-show="cardno">
          <img src="../../../assets/images/recharge/input_delete.png" />
        </div>
      </div>
      <ul>
        <li
          v-for="(list,index) in card_list"
          @click="pirce_check(list.id,list.price,3)"
          :class="{'check':cardno.length == 12}"
        >{{list.price}}</li>
      </ul>
      <div class="hint">温馨提示：充值完成后请到线下进行圈存</div>
    </div>
    <!-- 旅游卡充值(有充值记录) -->
    <div class="recordCard" v-show="tab_number == '2' && recharge == 1 && !first_card_show">
      <div class="title">
        <span>提示：</span>由于数据传输到系统存在一定延时，卡面余额仅作参考。
      </div>
      <div
        class="card_list"
        @click="swipeRight"
        :class="{height1:sctcdAccount.length == 1,height2:sctcdAccount.length == 2,height3:sctcdAccount.length > 2}"
      >
        <v-touch
          tag="div"
          ...
          class="list"
          :class="{left:index==sctcdAccount.length-1 && swipe_left == 1,right:index==sctcdAccount.length-1 && swipe_right == 1}"
          data-id="1"
          data-password="456789"
          v-for="(list,index) in sctcdAccount"
          :key="index"
        >
      ...
      <div class="alrCard_bg" v-show="alrCard_bg_show">
        <div class="alrCard">
          ...
          <img
            src="../../../assets/images/recharge/input_delete.png"
            @click="alrCard_bg_show = false"
          />
          <div
            class="list"
            v-for="(list,index) in card_list"
            @click="pirce_check(list.id,list.price,4)"
          >{{list.price}}</div>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      data-baidu-stats="话费充值-周五充值活动"
      @click="gotoActivity()"
      v-if="activityImgUrl && tab_number == 0 && isStartActivity"
    >
    ...
    <footer v-show="tab_number == '2' && recharge == 1 && !first_card_show">
      <div class="integral">
        可用积分：
        <span>{{toRecharge.availablePoint? toRecharge.availablePoint.toFixed(2) : '0.00'}}</span>
      </div>
    ...
    <div class="flow_bg" v-show="flow_bg_show">
      <div class="flow">
        ...
        <span>1、输入卡号</span>
        <br />
        <img src="../../../assets/images/recharge/flow_card.png" />
        <br />
        <span>2、选择金额，完成支付</span>
        <br />
...
export default {
  ...
  data() {
    return {
      ...
      novice: 0, //新手指引
      mobile:
        localStorage.mobile.substring(0, 3) +
        " " +
        localStorage.mobile.substring(3, 7) +
        " " +
        localStorage.mobile.substring(7, 11), //手机号展示
  ...
  watch: {
    ...
    sctcdAccount() {
      ...
      if (
        this.sctcdAccount[0].cardno != "" ||
        this.sctcdAccount[0].cardno != null
      ) {
  ...
  methods: {
    ...
    showOperator(item) {
      ...
      if (this.toRecharge.charges) {
        this.charge_text =
          "兜礼积分充值将收取" + this.toRecharge.charges + "%手续费，";
        if (this.toRecharge.daily_limit) {
          this.charge_text +=
            "每月积分最多兑换" + this.toRecharge.daily_limit + "元话费";
        } else {
          this.charge_text = this.charge_text.substring(
            0,
            this.charge_text.length - 1
          );
        }
      ...
      if (this.toRecharge.discounts_month_limit) {
        this.charge_text =
          "兜礼积分每月兑换" +
          this.toRecharge.discounts_month_limit +
          "元免手续费，超过" +
          this.toRecharge.discounts_month_limit +
          "元的部分，" +
          this.charge_text;
    ...
    binding_view() {
      dooolyAPP.gotoJumpVue.call(
        this,
        "/travelCard_details/" +
          this.sctcdAccount[0].idCard +
          "/" +
          this.sctcdAccount[0].userName +
          "/" +
          this.sctcdAccount[0].phoneno
      );
    },
    rechargeList() {
      dooolyAPP.gotoJumpVue.call(
        this,
        "/rechargeList/" + this.sctcdAccount[0].cardno
      );
    },
    rechargeHelpCenter() {
...
.recordCard ul li {
  ...
  width: 100%;
  border-bottom: 1px solid #ececec;
  background: url(../../../assets/images/recharge/right.png) no-repeat right
    0.32rem;
...
.coupon_div .coupon_fl {
  font-size: 0.28rem;
  height: 1rem;
  float: left;
  line-height: 1rem;
}
.coupon_div .coupon_fr {
  float: right;
  height: 1rem;
  line-height: 1rem;
  color: #ee3f44;
