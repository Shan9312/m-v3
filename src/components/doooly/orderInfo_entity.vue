  ...
  <div class="view">
    ...
    <header v-else @click="link()" class="clearfix">
      ...
      <div v-else-if="haveSotrageData == 1">
        <div class="fl clearfix">
          ...
          <span
            class="address"
          >收货地址：{{deliveryAddressData.province + deliveryAddressData.city + deliveryAddressData.area + deliveryAddressData.address}}</span>
        </div>
        <div class="fr">
          <img src="../../assets/images/userDelivery/right.png" />
        </div>
    ...
    <div class="main">
      ...
      <div class="list_li list_li_minute option clearfix store1" @click="store_list(1)">
        ...
        <div class="fl">办卡门店</div>
      </div>

      <!-- 办卡区域 -->
      <div
        class="list_li list_li_minute option clearfix store2"
        @click="distpicker(2)"
        v-if="this.postDataSpecification.indexOf('两店') > 0"
      >
        <div class="fr">
          <img src="../../assets/images/userDelivery/right.png" />
        </div>
      ...
      <div class="list_li list_li_minute clearfix picture">
        <div class="fl">身份证照片</div>

        <div
          class="center fl"
          @click="chooseImage(1)"
          v-if="browserName == 'WebKit'|| browserName == 'Chrome WebView' || browserName == 'otherAPPAndroid' || browserName == 'otherAPPIos'"
        >
        ...
        <div class="selectImgDiv" v-else>
          <div class="showImgDiv">
            <img
              v-if="headPhoto==null"
              style
              src="../../assets/images/userDelivery/addDefaultImg.png"
            />
            <img v-else :src="headPhoto" />
          </div>
      ...
      <div class="list_li list_li_minute clearfix picture">
        ...
        <div class="selectImgDiv" v-else>
          ...
          <input
            ...
            type="file"
            accept="image/*"
            @change="chooseWorkCardImg($event)"
          />
        </div>
      </div>
    </div>
    <div class="company">
      <div class="title">
        <span>﹡</span>在健身卡有效期内，持卡人在同一家门店只能拥有一张卡
      </div>
      <div class="clearfix picture_view">
        ...
        <div class="fl price">
          ...
          <br />
          <span class="type">规格：{{postData.specification}}</span>
          <br />
          <span>
    ...
    <footer>
      <div class="fl">
        ...
        <span>
          ...
          <span class="sellPrice">{{postData.sellPrice}}</span>
        </span>
      </div>
      <div class="fr" @click="refer()" :disabled="referDisable" :class="classObj">确认提交</div>
    </footer>
      ...
      <div class="store_list">
        ...
        <ul>
          <li
            ...
            @click="selectStore(item.id,item.storeName)"
          >{{item.storeName}}</li>
    ...
    <div v-if="distPopVisible" class="data_bg">
      <div class="data">
        ...
        <ul>
          <li v-for="(item,index) in districtData" @click="selectCity(index,item)">{{item}}</li>
        </ul>
      </div>
    </div>
  </div>
    ...
    referDisable: function() {
      if (this.postDataSpecification.indexOf("两店") > 0) {
        if (
          ...
          this.tel == "" ||
          this.card == "" ||
          this.card_picture_url == null ||
          this.picture_url == null ||
          this.empCard_url == null ||
          this.isRefering
        ) {
          return false;
        } else {
    ...
    getUserDelivery() {
      http({
        ...
        if (this.$cookies.get("addressDataList")) {
          this.userDeliveryList.push(
            data.data.userDeliveryList[this.$cookies.get("addressDataList")]
          );
          this.deliveryAddressId =
            data.data.userDeliveryList[this.$cookies.get("addressDataList")].id;
          this.deliveryName =
            data.data.userDeliveryList[
              this.$cookies.get("addressDataList")
            ].receiverName;
          this.deliveryPhone =
            data.data.userDeliveryList[
              this.$cookies.get("addressDataList")
          ...
          this.deliveryAddr =
            data.data.userDeliveryList[0].province +
            data.data.userDeliveryList[0].city +
            data.data.userDeliveryList[0].area +
            data.data.userDeliveryList[0].address;
        }
      });
    },
    link() {
        ...
        merchantProduct: [
          {
            ...
            productSku: [
              {
                productType: this.postData.productType,
                productId: this.postData.merchantProduct[0].productSku[0]
                  .productId,
