<template>
  <div>
    <div class="detail">
      <div class="sub-tc" v-html="adBusiness.noticeInfo"></div>
      <div class="footer_bg" :class="{'fix-iphonex-bottom': isIphoneX}"></div>
      <!-- 网易严选特殊处理 -->
      <footer class="yanxuan" :class="{'fix-iphonex-bottom': isIphoneX}" v-show="adBusiness.company === '网易严选'">
      ...
      <footer v-show="footer_type_show == 1 && adBusiness.company !== '网易严选'">
        <div class="fl" @click="judge(1)">
          {{footer_fl}}
          <span class="footer_tips" v-if="adBusiness.isSupportIntegral !== '1'">(该商户暂不可用兜礼积分)</span>
        </div>
        <div class="fr" @click="judge(2)">{{footer_fr}}</div>
      </footer>
      <footer v-show="footer_type_show == 2 && adBusiness.company !== '网易严选'" @click="judge(1)">
        {{footer_fl}}
        <span class="footer_tips single_btn" v-if="adBusiness.isSupportIntegral !== '1'">(该商户暂不可用兜礼积分)</span>
      </footer>
    ...
    <toast v-if="toastState == 1" :toastData="toastData" :firstShow="firstShow" v-on:child-say="listenToMyBoy" ></toast>
    ...
    <div class="JDK_alr_bg" v-show="lod_logo_show == '1'">
      <div class="JDK_alr">
        <div class="img">
          ...
          <span><img class="JDK_logo" :src="toastData.miniLogo" :onerror="defaultImg"></span>
        </div>
        <div class="txt">
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      //区分APP与安卓
      if(this.$browserName == "WebKit" || this.$browserName == "Chrome WebView" || this.$browserName == "otherAPPAndroid" || this.$browserName == "otherAPPIos"){
        this.channel = 0;
      }else{
        this.channel = 1;
      }
      ...
      adBusiness(){
        ...
        if(this.$browserName == "WeChat"){
          if(this.adBusiness.company=='威尔士'){
            Wechatshare('hotbusiness',[this.adBusiness.id,this.adBusiness.dealType]);
          }
        };
        //滴滴出行积分验证
        ...
        this.toastData.url = this.adBusiness.sWechatJumpUrl;
        ...
        if (this.toastData.maxUserRebate == '' || this.toastData.maxUserRebate == '0') {
          this.toastData.maxUserRebate = '';
        }else{
          this.toastData.maxUserRebate = '最高返' + this.toastData.maxUserRebate + '%';
        };
        //判断底部按钮显示类型
    ...
    methods:{
      ...
      checkButtonType(){
        //判断底部按钮显示类型
        if(this.adBusiness.dealType == 0){//线上
          if(this.channel == 0){//线上app端
            if(this.productId > 0){
        ...
        }else if(this.adBusiness.dealType == 1){//线下
          if(this.productId > 0){
        ...
        //进行url非空校验
        if(this.footer_type == 1 || this.footer_type == 3 || this.footer_type == 5 || this.footer_type == 6 || this.footer_type == 8 || this.footer_type == 10){
        ...
        if(this.footer_type == 2 || this.footer_type == 4 || this.footer_type == 7 || this.footer_type == 9 ||this.footer_type == 5 || this.footer_type == 10){
      ...
      judge(type){
        ...
        if(!this.diditaost){
          this.$messageBox({
            title: '无法享受会员权益',
            message: '由于您已加入其他企业享受企业用车服务，无法与兜礼企业用车服务同享。请在滴滴出行企业版APP中修改企业用车权限。',
            confirmButtonText: '我知道了',
          });
          return;
        };
        if(type == 1){//左边按键点击事件
          if(this.footer_type == 1 || this.footer_type == 3 || this.footer_type == 5 || this.footer_type == 6 || this.footer_type == 8 || this.footer_type == 10){
            this.business();
          }else if(this.footer_type == 2 || this.footer_type == 4 || this.footer_type == 7 || this.footer_type == 9){
            this.link(this.adBusiness.sAppJumpUrl,1);
          }else if(this.footer_type == 11 || this.footer_type == 12 || this.footer_type == 13){
            this.businessDetail();
          };
          // baiduStats('商户中心-' + this.adBusiness.company + '-左边按钮');
          let btnName = this.footer_fl;
          if (!this.footer_fl) {
            btnName = '前往h5'
          }
          this.$baiduStats('商户中心-' + this.adBusiness.company + '-' + btnName, this.adBusiness.company);
        }else if(type == 2){//右边按键点击事件
          ...
          // baiduStats('商户中心-' + this.adBusiness.company + '-右边按钮');
          let btnName = this.footer_fl;
          if (!this.footer_fl) {
            btnName = '使用APP'
          }
      ...
      business(){
        if(this.adBusiness.openOneNnumber == 1){
          ...
          if(document.title == "京东返利"){
            if(this.toastData.discount != '' && this.toastData.discount != '0' && this.toastData.maxUserRebate != '' && this.toastData.maxUserRebate != '0'){
      ...
      gotoJumpJq1(url){
        ...
        if(this.$browserName == "WebKit" || this.$browserName == "otherAPPIos" || this.$browserName == "Chrome WebView" || this.$browserName == "otherAPPAndroid"){
      ...
      link(link,type,id) {
        // 跳转线上app特殊处理2\4\5
        if(this.footer_type === 2 || this.footer_type === 4 || this.footer_type === 5){
          dooolyAPP.androidGoMerchantApp(this.adBusiness.company,this.adBusiness.appUrl)
          return false
        }else{
          if(type==1){
...
<style scoped>
  ...
  .footer_bg{width: 100%;height: 1rem;}
