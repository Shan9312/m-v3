<template>
  <div class="bg">
    <header><img src="../../assets/images/personalCenter/accountSecurity/tel.png" /></header>
    <section class="section_form">
      <article class="clearfix tel">
        <div class="index_div">
          <input type="tel" v-model="mobileNum"  class="form-control" maxlength="11"
                 name="loginMobile" placeholder="请输入新手机号码">
        </div>
      </article>
      ...
      <div class="btn_div_d">
        <button :disabled="nextStep" @click="okBtn()" :class="classObject">确认</button>
      </div>
    </section>
  ...
  export default {
    ...
    methods:{
      checkMobile() {
        ...
        else if (checkMobile(this.mobileNum)) {
          this.$toast("手机号格式错误！");
          return false;
        }
        else {
      ...
      getValidCode:function () {
        ...
        http({
          method: 'post',
          url: api.modifyMobileGetCode+"?mobile="+this.mobileNum,
        }).then((res) => {
      ...
      okBtn:function () {
        ...
        http({
          method: 'post',
          url: api.modifyMobileStepTwo+"?mobile="+this.mobileNum+"&code="+this.validCodeNum,
        }).then((res) => {
            if (res.data.code == "1001") {
              localStorage.mobile=this.mobileNum;
              this.$toast("密保手机更换成功，请注意账号安全");
              dooolyAPP.jumpPersonPage();
            }else {
