<template>
  <div class="view" v-show="loding">
    ...
    <slot name="header">
      <header>
        <img :class="{ min_logo: min_logo }" :src="headerImgSrc" />
      </header>
    </slot>
    ...
    <section class="logo_main" :class="{ dahuaAPP: !dahuaShow }">
      <form @focusin="focusinMethod" @focusout="focusoutMethod">
        ...
        <div id="log1" v-if="mobileState">
          ...
          <article class="clearfix" v-show="!dahuaShow">
            <span>温馨提示：</span
            >本次您所登录的手机号需要与您经常使用电商平台的手机号保持一致，便于积分使用。
          </article>
          <article class="clearfix">
            <div class="index_div">
              <input
                type="tel"
                v-model="validCodeNum"
                placeholder="请输入验证码"
                class="form-control"
                maxlength="6"
                id="code"
                name="code"
              />
              <div class="code_btn_txt" @click="getValidCode" v-if="codeState">
                获取验证码
              </div>
              <div class="code_btn_txt" v-if="codeState2">获取验证码</div>
              <div class="code_btn_d" v-if="!codeState && !codeState2">
                {{ timeNum }}s后重新获取
              </div>
            </div>
          </article>
        ...
        <div id="log2" v-if="accountState">
          ...
          <article class="clearfix">
            <div class="index_div">
              <input
                v-model="passwordNum"
                class="passwordType form-control"
                maxlength="6"
                minlength="6"
                type="tel"
                id="password"
      ...
      <div class="deal_div clearfix">
        <div class="fl" v-if="typeAPP">
          <a data-baidu-stats="会员登录-用户协议" @click="user_agreement"
            >用户登录即代表同意<span>《用户协议》</span></a
          >
        </div>
        <div class="fr">
          <div
            @click="forgetPwd"
            class="disableCss"
            data-baidu-stats="会员登录-忘记密码"
            id="btn_pwd"
            v-show="!mobileState && dahuaShow"
          >
            忘记密码？
          </div>
        </div>
    ...
    <div
      class="footer-wrapper clearfix"
      @click="changeLogin"
      v-show="dahuaShow"
    >
      <div class="btn_block" id="cardLoginDiv" v-if="mobileState">
        账号密码登录
      </div>
    ...
    <div class="coverBg" v-if="nonentityVisible">
      <div class="alt_nonentity">
        ...
        <div class="model_btn_div">
          <button type="button" @click="cancelActivateBtn()" class="cancel">
            取消
          </button>
          <button type="button" @click="confirmActivateBtn()" class="active">
            激活
          </button>
        </div>
      </div>
      ...
      <div class="alt1">
        ...
        <div class="btn_div">
          <input
            ...
            class="cancel"
            value="否"
            @click="alr_show = false"
          />
          <input
            type="button"
            class="active"
            value="是"
            @click="confirmFindPSW()"
          />
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted () {
    document.getElementsByClassName('view')[0].style.height =
        window.innerHeight + 'px';
    this.screenHeight =
        document.getElementsByClassName('view')[0].offsetHeight -
        document.getElementsByClassName('footer-wrapper')[0].offsetHeight * 3 -
        document.getElementsByClassName('logo_main')[0].offsetHeight -
        document.getElementsByClassName('header_top')[0].offsetHeight -
        document.getElementsByClassName('header_bottom')[0].offsetHeight -
        document.getElementsByTagName('header')[0].offsetHeight
      if (this.screenHeight != '') {
      document.getElementsByClassName('footer_fixed')[0].style.height =
    ...
    let md = new MobileDetect(window.navigator.userAgent)
      ...
      headers: {
        appSource: 'H5', // 渠道
        deviceId:
            getDeviceId() == 'undefined' ? getDeviceId() : localStorage.userId, // 设备id
  ...
  methods: {
    ...
    altAwaitActive (date) {
      this.$messageBox(
        ...
        '您已经在' +
            date +
            '提交过申请，我们会在5个工作日内完成审核。如有疑问，您可以拨打客服电话400-158-2212(' +
            dooolyConfig.serviceTime +
            ')'
      )
      },
    // 审核未通过
    ...
    altAccountError () {
      this.$messageBox({
        title: '账号异常',
        message:
            '亲爱的会员，您的账号存在异常，请拨打客服电话“400-158-2212”进行处理 (' +
            dooolyConfig.serviceTime +
            ')',
    ...
    mobileLogin () {
      ...
      this.isLoading = true
        ...
        data: {
          ...
          isSimpleAtuoRegister: urlData
            ? urlData.isSimpleAtuoRegister
            : undefined,
  ...
  created () {
    ...
    this.appInit()
      ...
      http({
        ...
        data: {
          thirdUserToken:
              sessionStorage.oauthCode || localStorage.thirdUserToken,
