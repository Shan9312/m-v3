  ...
  <div class="view" v-show="loding">
    ...
    <section class="logo_main" :class="{'dahuaAPP':!dahuaShow}">
      <form @focusin="focusinMethod" @focusout="focusoutMethod">
        ...
        <div id="log1" v-if="mobileState">
          ...
          <article class="clearfix">
            <div class="index_div">
              <input type="tel" v-model="validCodeNum" placeholder="请输入验证码" class="form-control" maxlength="6" id="code" name="code">
        ...
        <div id="log2" v-if="accountState">
          ...
          <div class="btn_div_d">
            <button type="button" class="logo_btn btn" @click="accountLogin" :disabled="log2Disable" id="log2_dl" :class="classObject2">
              登录
            </button>
          </div>
        </div>
      ...
      <div class="deal_div clearfix">
        <div class="fl" v-if="typeAPP"><a data-baidu-stats="会员登录-用户协议" @click="user_agreement" >用户登录即代表同意<span>《用户协议》</span></a>
        </div>
        <div class="fr">
          <div @click="forgetPwd" class="disableCss" data-baidu-stats="会员登录-忘记密码" id="btn_pwd" v-show="!mobileState && dahuaShow">忘记密码？</div>
        </div>
      </div>
    </section>
