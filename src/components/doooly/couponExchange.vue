  ...
  <div>
    <header>
      <div class="clearfix">
        <div class="fr">
          <span class="logo">
            <img :src="couponDetail.adBusiness.logo" :onerror="defaultImg" />
          </span>
          <br />
          <span class="company">{{couponDetail.coupon.businessName}}</span>
        </div>
        <span class="coupon">{{couponDetail.coupon.productName}}</span>
        <br />
        <span
          class="price"
          v-if="couponDetail.coupon.couponValue > 0 && couponDetail.coupon.kind==1"
        >
          <b>{{couponDetail.coupon.couponValue}}</b>
          <span v-if="couponDetail.coupon.kind==0">元</span>
          <span v-else-if="couponDetail.coupon.kind==1">折</span>
        </span>
        <br />
    ...
    <section>
      <ul>
        <li class="clearfix explain">
          ...
          <p
            v-html="couponDetail.coupon.wechatConvertMethod"
            v-if="explainState"
            class="newExplain"
          ></p>
        </li>
        ...
        <li
          v-if="couponDetail.adBusiness.dealType==1"
          class="clearfix"
          @click="businessDetail(couponDetail.adBusiness.id,couponDetail.adBusiness.company)"
        >
          <span>适用门店(线下)</span>
          <div class="fr">
            <i class="icon iconfont">&#xe804;</i>
    ...
    <div v-if="popupVisible" class="popup_bg">
        ...
        <div class="coupon_div">
          ...
          <div class="bottom_btn">
            <div v-if="couponDetail.adBusiness.dealType==0">
              ...
              <input
                class="last_child"
                @click="link(couponDetail.coupon.businessOnlineUrl)"
                type="button"
                value="去使用"
              />
            </div>
      ...
      <div v-if="usualPopupVisible" class="usualPopup">
        ...
        <div
          ...
          class="coupon"
        >
          <span class="coupon_title">兑换码</span>
          <span id="copyCode" class="detail">{{couponDetail.couponCode}}</span>
          <div class="bottom_btn">
            <div v-if="couponDetail.adBusiness.dealType == 0">
              <input
                class="confirm copy_btn"
                data-clipboard-action="copy"
                data-clipboard-target="#copyCode"
                type="button"
                :value="couponDetail.coupon.businessOnlineUrl ? '复制去使用' : '复制'"
                @click="copyBtn()"
              />
            </div>
            <div v-else>
...
export default {
  ...
  methods: {
    link() {
      ...
      this.$baiduStats(
        "我的福利-点击去使用-" + this.couponDetail.coupon.productName
      );
    },
    ...
    exchangeCard: function() {
      ...
      this.$baiduStats(
        "我的福利-点击立即兑换-" + this.couponDetail.coupon.productName
      );
    },
    //复制到剪贴板
    copyBtn() {
      ...
      this.$baiduStats(
        "我的福利-点击复制去使用-" + this.couponDetail.coupon.productName
      );
    },
    loadPageData() {
      http({
...
@font-face {
  ...
  src: url("//at.alicdn.com/t/font_402124_r3z8okj55855ewmi.eot?#iefix")
    ...
    url("//at.alicdn.com/t/font_402124_r3z8okj55855ewmi.svg#iconfont")
      format("svg");
}
...
header {
  background: #fff url(../../assets/images/coupon/to_use_banner.png) center top
    no-repeat;
...
.popup_bg .cardPopup .coupon_div .coupon {
  padding-top: 0.7rem;
}
.popup_bg .cardPopup .coupon_div .coupon .left {
  line-height: 0.6rem;
  text-align: right;
}
.popup_bg .cardPopup .coupon_div .coupon .left span {
  line-height: 0.6rem;
  font-size: 0.28rem;
  color: #999999;
}
.popup_bg .cardPopup .coupon_div .coupon .left span:last-child {
  margin-top: 0.3rem;
}
.popup_bg .cardPopup .coupon_div .coupon .right {
  line-height: 0.6rem;
  text-align: left;
  margin-left: 0.2rem;
}
.popup_bg .cardPopup .coupon_div .coupon .right span {
  padding: 0 0.3rem;
  line-height: 0.6rem;
  font-size: 0.32rem;
  color: #333333;
...
.popup_bg .cardPopup .coupon_div .coupon .right span:last-child {
  margin-top: 0.3rem;
}
...
.popup_bg .usualPopup .close {
  ...
  right: -0.6rem;
  top: -0.6rem;
  background: rgba(0, 0, 0, 0) url(../../assets/images/coupon/closeNew.png)
    no-repeat 0.25rem 0.75rem;
  width: 1.2rem;
  height: 1.2rem;
  border-radius: 100%;
...
.popup_bg .usualPopup .coupon .detail {
  margin-left: 0.2rem;
  padding: 0 0.3rem;
  background-color: #f5f5f5;
  border-radius: 0.1rem;
}
.popup_bg .usualPopup .coupon .coupon_title {
  font-size: 0.28rem;
  color: #999;
}
