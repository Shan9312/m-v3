  ...
  <div class="view">
    ...
    <div class="charge tab_list">
      ...
      <div class="header-title">
        兑换集享积分
        <span>（在兜礼兑换集享卡积分比例为：1:100）</span>
      </div>
      <ul>
        <li
          v-for="(item,index) in list.sku_list"
          :class="{check:newMobile.length == 11,click:click == index && newMobile.length == 11}"
          @click="checklist(index,item.price)"
        >{{item.specification}}</li>
      </ul>
      <div class="hint">
        <div class="title">兑换使用说明</div>1、首次兑换时需要绑定集享卡账号；
        <br />2、绑定的集享卡账号将从兜礼会员账号中扣减兜礼积分；
        <br />3、兑换的集享卡积分有效期至次年12月31日。
      </div>
    </div>
    <div class="toast footer_toast">
      <span @click="first_card_show = !first_card_show">使用流程</span>&nbsp;&nbsp;&nbsp;&nbsp;
      <span @click="helpCenter">常见问题</span>
    </div>
    ...
    <footer>
      <div class="integral">
        合计：
        <span style="color:#ee3f44;">{{integral}}</span>
      </div>
      <div
        class="button"
          ...
          if (data.data.code == 1000) {
            ...
            if (res.data.data.zeroOrderFlag) {
              ...
              if (this.firstBindQuery) {
                if (
                  window.location.href.indexOf("pre_dist") > 0 ||
                  window.location.href.indexOf("pro_dist") > 0
                ) {
                  dooolyAPP.gotoJumpJq.call(
                    this,
                    "http://api.maxxipoint.com/NexusService/auth.do?response_type=token&client_id=74D33C18BF3811B2&redirect_uri=" +
                      encodeURIComponent(api.baseURL) +
                      "dist%2f%23%2ffamilyTo%2f" +
                      this.orderNum +
                      "&status=199210219"
                  );
                } else {
                  dooolyAPP.gotoJumpJq.call(
                    this,
                    "http://app-uat.maxxipoint.com/NexusService/auth.do?response_type=token&client_id=844AB181D6878FF9&redirect_uri=" +
                      encodeURIComponent(api.baseURL) +
                      "dist%2f%23%2ffamilyTo%2f" +
                      this.orderNum +
                      "&status=199210219"
